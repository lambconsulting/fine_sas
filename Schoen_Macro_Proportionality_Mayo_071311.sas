/*------------------------------------------------------------------*
   ...
   |               from the Cox model.
   *------------------------------------------------------------------*
   | CREATED BY  : Bergstralh, Erik              (04/14/2004 11:50)
   |             : Therneau, Terry
   *------------------------------------------------------------------*
   | PURPOSE
   |
   ...
   *------------------------------------------------------------------*
   | MODIFIED BY : Lennon, Ryan                  (01/25/2011 15:13)
   |
   | Corrected upper/lower case bug for comparing the XVARS and PVARS lists.
   *------------------------------------------------------------------*
...
%LOCAL BAD BADALPHA DOALPHA BADSMOOTH NXS PLOTVARS NPLOTVAR PV FOUND
   ...
   %do i=1 %to &nplotvar;
      ...
      %let found=0;
      %do j=1 %to &nxs;
         %if %upcase(&pv)=%upcase(%scan(&xvars,&j,' '))
           %then %let found=1;
      %end;
      %if &found=0 %then %do;
